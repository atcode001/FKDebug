...
function showMethods(postdata){
    ...
    Java.perform(function(){
        ...
        Java.enumerateLoadedClassesSync().forEach(function(className){
            ...
            if(!hasMethod){
                cnt+=1;
                fKLog.kLog("className:"+className)
                return;
            }
            ...
            try{
                ...
                for(var i=0;i<methods.length;i++){
                    ...
                    cnt+=1;
                    fKLog.kLog("className:"+className+"----method:"+methodName)
                }
            }catch(ex){

            }
        })
        fKLog.kLog("find count:"+cnt);
    });
}
//查找so的符号
    ...
    Process.enumerateModules().forEach(function(module){
        ...
        if(showType=="Export"){
            module.enumerateExports().forEach(function(edata){
                ...
                cnt+=1;
                fKLog.kLog("module:"+module.name+"----exportName:"+edata.name+"----address:"+edata.address+"----type:"+edata.type)
            });
        }else{
            module.enumerateSymbols().forEach(function(edata){
                ...
                cnt+=1;
                fKLog.kLog("module:"+module.name+"----symbolName:"+edata.name+"----address:"+edata.address+"----type:"+edata.type)
            });
        }
    });
}
...
function dumpSo(postdata){
    Java.perform(function () {
        ...
        fKLog.kLog("[name]:"+ libso.name);
        fKLog.kLog("[base]:"+ libso.base);
        fKLog.kLog("[size]:"+ ptr(libso.size));
        fKLog.kLog("[path]:"+ libso.path);
...
function searchInfo(postdata){
    Java.perform(function(){
        ...
        fKLog.kCLog("appinfo_search count:" + count, "appinfo_search", JSON.stringify(appinfo))
    });
}
...
function loadAppInfo(){
    Java.perform(function(){
        ...
        appinfo["spawn"] = "%spawn%"
        fKLog.kCLog("加载appinfo", "appinfo", JSON.stringify(appinfo))
    })
}

    function main() {
        fKLog.kCLog("", "init", "default.js init hook success")
    loadAppInfo();
    recvMessage();
}
setImmediate(main);
